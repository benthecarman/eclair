...
import fr.acinq.eclair.payment.relay.Relayer.ForwardFulfill
...
class PaymentLifecycle(nodeParams: NodeParams, cfg: SendPaymentConfig, router: ActorRef, register: ActorRef) extends FSMDiagnosticActorLogging[PaymentLifecycle.State, PaymentLifecycle.Data] {
  ...
  when(WAITING_FOR_PAYMENT_COMPLETE) {
    ...
    case Event(ff: ForwardFulfill, WaitingForComplete(s, c, cmd, _, _, _, _, route)) =>
      val p = PartialPayment(id, c.finalPayload.amount, cmd.amount - c.finalPayload.amount, ff.htlc.channelId, Some(cfg.fullRoute(route)))
      onSuccess(s, cfg.createPaymentSent(ff.paymentPreimage, p :: Nil))
      myStop()
