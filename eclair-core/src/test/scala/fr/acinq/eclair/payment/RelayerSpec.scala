...
class RelayerSpec extends TestkitBaseClass {
  ...
  def testRelayTrampolineHtlcFulfill(f: FixtureParam, onChain: Boolean): Unit = {
    ...
    val fulfill_bc = UpdateFulfillHtlc(channelId_bc, 72, preimage)
    val forwardFulfill: ForwardFulfill = if (onChain) {
      ForwardOnChainFulfill(preimage, TrampolineRelayed(null, Some(payFSM.ref)), add_bc)
    } else {
      ForwardRemoteFulfill(fulfill_bc, TrampolineRelayed(null, Some(payFSM.ref)), add_bc)
    }
  ...
  test("relay a trampoline htlc-fail") { f =>
    ...
    sender.send(relayer, ForwardRemoteFail(fail_ba, origin, add_bc))
    payFSM.expectMsg(fail_ba)
    payFSM.expectNoMsg(100 millis)

    // same for an on-chain downstream failure.
    val onChainFailure = HtlcsTimedoutDownstream(channelId_bc, Set(add_bc))
    sender.send(relayer, ForwardOnChainFail(onChainFailure, origin, add_bc))
    payFSM.expectMsg(Status.Failure(onChainFailure))
    payFSM.expectNoMsg(100 millis)
  }
