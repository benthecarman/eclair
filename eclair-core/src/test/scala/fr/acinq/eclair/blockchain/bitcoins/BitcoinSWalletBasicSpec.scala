...
class BitcoinSWalletBasicSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  def initWallet: Future[BitcoinSWallet] = {
    ...
    for {
      ...
      addr <- started.getFinalAddress
      // fixme kinda hacky, but this way we only process confirmed blocks and don't spend immature coinbases
      hashes <- started.extendedBitcoind.generateToAddress(10, BitcoinAddress.fromString(addr))
      _ <- started.extendedBitcoind.generateToAddress(101, BitcoinAddress.fromString(addr))
      _ <- started.extendedBitcoind.downloadBlocks(hashes.map(_.flip))
    } yield started
  }
